    ...
    <div data-aos="fade-up" class="container mx-auto p-5">
        ...
        <div class="max-w-screen-lg p-3 mx-auto bg-white rounded-lg shadow-md overflow-hidden mb-5">
            ...
            <div class="bg-sky-600 rounded-3xl px-4 bg-opacity-55 py-2 flex justify-between items-center">
                ...
                <a href="/users" class="text-gray-300 hover:text-white"><svg class="w-6 h-6" fill="none"
            ...
            <div class="p-4 grid grid-cols-1 sm:grid-cols-2">
                <div class="flex items-center justify-center mb-4">
                    <img src="{{ asset('pictures/' . $user->pictures) }}" alt="Foto Profil"
                        class="w-40 h-40 object-cover rounded-md">
                </div>
                <div class="mb-2">
                    <h2 class="text-xl font-semibold">{{ $user->username }}</h2>
                    <p class="text-gray-600 text-sm"><span>Email: -</span>{{ $user->email }}</p>
                    <p class="text-gray-600 text-sm"><span>Nama lengkap: -</span>{{ $user->nama_lengkap }}</p>
                    <p class="text-gray-600 text-sm"><span>Tanggal lahir: -</span>{{ $user->tanggal_lahir }}</p>
                    <p class="text-gray-600 text-sm"><span>Alamat : -</span>{{ $user->alamat }}</p>
                    <p class="text-gray-600 text-sm"><span>Jenis kelamin: -</span>{{ $user->jenis_kelamin }}</p>
                    <p class="text-gray-600 text-sm"><span>Bio : -</span>{{ $user->bio }}</p>
                </div>
                <!-- Tab Navigation -->
            </div>
        </div>
                        ...
                        <div class="flex items-start gap-2.5 mb-4">
                            <div
                                ...
                                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                    <span
                                        class="text-sm font-semibold text-gray-900 dark:text-white">{{ $item->users->username }}</span>
                                    <span
                                        class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ $item->created_at->format('H:i') }}</span></span>
                                </div>
                                <p class="text-sm font-normal py-2.5 text-gray-900 dark:text-white">{{ $item->isi_komen }}
                                </p>
                            </div>
                        </div>
                    @endforeach
