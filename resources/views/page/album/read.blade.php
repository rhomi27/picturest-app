        ...
        @if (auth()->check() && $item->user_id == auth()->id())
            <div class="flex justify-end gap-3 items-center p-2 px-5 bg-gray-200">
                @if (Auth::check() && Auth::user()->status === 'banned')
                    <button disabled type="button" data-id="{{ $item->id }}"
                        class="flex items-center cursor-pointer open-modal">
                        <svg class="w-4 h-4 text-black" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                ...
                <button class="flex items-center delete" data-id="{{ $item->id }}" data-nama="{{ $item->nama }}">
                    <svg class="w-4 h-4 text-red-600" xmlns="http://www.w3.org/2000/svg" id="Outline"
